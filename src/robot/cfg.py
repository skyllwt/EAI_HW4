...
class RobotCfg:
    path_urdf: str = None
    """path to urdf file"""
    head_path_urdf: str = None
    """path to urdf file for head"""
    path_mjcf: str = None
    """used for mujoco"""
    ...
    camera_cfg: List[CameraCfg] = None
...
HEAD_CAMERA = CameraCfg(
    width=1280,
    height=720,
    intrinsics=np.array([[909.3706665039062, 0, 655.8588256835938], [0, 909.1517944335938, 376.8585205078125], [0, 0, 1]]),
    extrinsics=np.array(
        [
            [-0.00799401, -0.47236274,  0.88136799, 0.09511180089778923],
            [ 0.02866524,  0.88092569,  0.47238569, -0.07063164032376867],
            [-0.9995571 ,  0.02904088,  0.00649826, 0.01602290270257442],
            [0.0, 0.0, 0.0, 1.0],
        ]
    ),
    link_name="head_link2",
)
...
GalbotCfg = RobotCfg(
    ...
    head_path_urdf=os.path.join("asset", "robot", "galbot", "galbot_head.urdf"),
    ...
    camera_cfg=[HEAD_CAMERA, WRIST_CAMERA],
    gripper_width_to_angle_depth=GALBOT_GRIPPER_WIDTH_TO_ANGLE_DEPTH,
)

DefaultQuadCfg = QuadCfg(
    path_mjcf=os.path.join("asset", "robot", "unitree_go2", "xmls", "go2_mjx_fullcollisions_with_april.xml"),
    path_policy=os.path.join("asset", "model_weights", "go2_loco_v9.onnx"),
    joint_init_qpos=np.array(
        [
            0.1, 0.9, -1.8,
            -0.1, 0.9, -1.8,
            0.1, 0.9, -1.8,
...
def get_quad_cfg() -> QuadCfg:
    return deepcopy(DefaultQuadCfg)

# QUAD_ROBOT_PATH_MJCF = os.path.join("asset", "robot", "unitree_go2", "xmls", "go2_mjx_fullcollisions_with_aruco.xml")

# def get_quad_robot_path_mjcf() -> str:
#     return QUAD_ROBOT_PATH_MJCF